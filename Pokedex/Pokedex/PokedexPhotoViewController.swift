//
//  PokedexPhotoViewController.swift
//  Pokedex
//
//  Created by jyj on 9/29/25.
//
...
private extension PokedexPhotoViewController {
    @objc func deleteButtonPressed(_ sender: UIButton) {
        let alertVC = AlertViewController()
        alertVC.tag = PokedexAlertType.deletePhoto
        ...
        PHPhotoLibrary.requestAuthorization { [weak self] status in
            DispatchQueue.main.async {
                if status == .authorized {
                    ...
                    let alertVC = AlertViewController()
                    alertVC.tag = PokedexAlertType.goSetting
                    alertVC.alertType = .confirm
    ...
    func downloadPhoto() {
        ...
        let alertVC = AlertViewController()
        alertVC.tag = PokedexAlertType.downloadFinished
...
extension PokedexPhotoViewController: AlertViewControllerDelegate {
    func buttonPressed(buttonType: AlertButtonType, tag: Int?) {
        if buttonType == .ok {
            if tag == PokedexAlertType.deletePhoto {
                deletePhoto()
            }
            if tag == PokedexAlertType.goSetting {
